...
public class WatcherPlayer extends AgentWatcher<WatcherPlayerType> implements
  ...
  public WatcherPlayer(Player player) {
    super(WatcherPlayerType.builder()
            .factKeys(new HashSet<>(Arrays.asList(
//            AVAILABLE_MINERALS, ENEMY_RACE, AVAILABLE_GAS, POPULATION_LIMIT, POPULATION,
//            IS_PLAYER
            )))
            .factSetsKeys(new HashSet<>(Arrays.asList(
//            UPGRADE_STATUS, TECH_TO_RESEARCH, OUR_BASE, ENEMY_BASE,
...
//                  .filter(
//                      aTechTypeWrapper -> !p.getPlayer().hasResearched(aTechTypeWrapper.getType()))
//                  .collect(Collectors.toSet()));
